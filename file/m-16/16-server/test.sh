...
function checkOSNavailability(){
        ...
        do
		 ...
		 sshpass -p "user" ssh -o StrictHostKeyChecking=no 192.168.1.110 "docker logs  orderer0.example.com 2>&1 | grep "Start phase completed successfully" > /dev/null"
                 test $? -eq 0 && let rc=0
        done
        if [ $rc -ne 0 ]; then
...
function joinchannel(){
  ...
  -H "content-type: application/json" \
  -d '{
	"peers": ["192.168.1.113:7051","192.168.1.114:7051"]
  }'
  echo
  echo
}
